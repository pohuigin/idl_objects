...
;
; Name        : VERTIGO__DEFINE
;
; Purpose     : Define a VERTIGO IDL object. This object is to be inherited by 
;               specific data objects. Instead of having to design an object 
;               from scratch, VERTIGO's general methods may be inherited, and 
;               only specific archive directories and plotting configurations 
;               need be written into the VERTIGO::INIT method, within the 
;               SAT_PROP settings structure. 
;               
;               VERTIGO utilizes Andre Csillaghy's Framework object to great 
;               benefit.
;
; Category    : Ancillary Synoptic Objects
;
; Syntax      : IDL> VERTIGO=obj_new('VERTIGO')
;
; Example     : IDL> VERTIGO=obj_new('VERTIGO')
;               IDL> VERTIGO->plot
;
; Notes       : 1. On its own, VERTIGO will not run. It must be inherited by 
;                  another object.
;
; History     : 11-JUN-2009 Written, Paul Higgins, (ARG/TCD)
;
; Tutorial    : Not yet. For now take a look at the configuration section of 
;               http://solarmonitor.org/objects/synimon/
;
...
;----------------------------------------------->
;-- The help procedure, sampling VERTIGO's object commands

pro VERTIGO::help

print,' '
print,'*** The Versus Time IDL Object ***'
print,'Astrophysics Research Group - Trinity College Dublin'
print,' '
print,'Version: JUN.11.2009 - Written: Jun 11 2009 - P.A. Higgins'
print,' '
print,'Temporary tutorial may be found at: http://solarmonitor.org/objects/synimon'
print,' '
print,'General Object Commands:'
print,' '
print,"IDL> VERTIGO = obj_new('VERTIGO')              ;-- Creates the VERTIGO object."
print,"IDL> files = VERTIGO->list(time='4-jun-2007')  ;-- Lists the files in the given range."
print,"IDL> VERTIGO->read,time='4-jun-2007'           ;-- Reads the files in the given range into the object."
print,"IDL> data = VERTIGO->getdata('4-jun-2007')     ;-- Retrieves the data and, optionally, the headers and" 
print,"                                               ;-- file names saved in the object"
print,"IDL> maps = VERTIGO->getmap('4-jun-2007')      ;-- Retrieves the data maps saved in the object."
print,"IDL> time = VERTIGO->get(/time)                ;-- Retrieves the value of the specified keyword."
print,"IDL> VERTIGO->latest                           ;-- Reads the latest data file available for VERTIGO."
print,"IDL> VERTIGO->plot,time='30-may-2007 00:00:00' ;-- Plots the data set with the date closest to that" 
print,"                                               ;-- specified."
print,"IDL> obj_destroy,VERTIGO                       ;-- Destroys the object, freeing precious memory."
print,' '
...
FUNCTION VERTIGO::INIT, SOURCE = source, _EXTRA=_extra

RET=self->Framework::INIT( CONTROL = VERTIGO_control(), $
...
sat_prop={explot:{log:1, color:0}, $
	plot_prop:{log:1, color:254}, $
	fspan:{url:'http://solarmonitor.org',ftype:'*.fts*',path:'/objects/testdata/'}, $
...
PRO VERTIGO::Process,_EXTRA=_extra
...
if keyword_set(filelist) then begin 
	if var_type(*(self.filelist)) ne 0 then return, *(self.filelist) else return,'' & endif
if keyword_set(filescopied) then begin
	if var_type(*(self.filescopied)) ne 0 then return, *(self.filescopied) else return,'' & endif
if keyword_set(filesread) then begin
	if var_type(*(self.filesread)) ne 0 then return, *(self.filesread) else return,'' & endif
if keyword_set(index) then begin
	if var_type(*(self.index)) ne 0 then return, *(self.index) else return,'' & endif
...
;if var_type(property) eq 8 then property=property.(0)
...
function VERTIGO::data_process,maparr
...
function VERTIGO::point_offset, index, xoff, yoff
...
function VERTIGO::compensate,maparr,xoff=xoff,yoff=yoff
...
pro VERTIGO::multiplot,_extra=_extra,pmulti=pmulti,wsize=wsize,wset=wset, timerange=timerange, outpmulti, numplots
...
pro VERTIGO::movie,_extra=_extra,xsize=xsize,ysize=ysize,title=title, $	
...
if var_type(mapset[0]) ne 8 then return 
...
;if not keyword_Set(title) then title='VERTIGO -> Movie'
...
pro VERTIGO::plotman,timerange=timerange,filelist=filelist,xtitle=xtitle,ytitle=ytitle,title=title,_extra=exset
...
function VERTIGO::getdataext,dataset
...
function VERTIGO::getmap,index,fnames,filelist=filelist,timerange=timerange
...
pro VERTIGO::latest,filelist,fileslist
...
pro VERTIGO::setplotspecs,imagemap,_extra=exset
...
;imagemap={t:fltarr(100), y1, y2, index}

imagemap=imagemap[0]
imagemap=self->data_process(imagemap)

;utplot,data,_extra=ex
...
function VERTIGO::extractplot,image
...
function VERTIGO::fileortime,filetime
...
function VERTIGO::checkreaddata,filelist
...
pro VERTIGO::plot,timerange=timerange,filelist=filelist,latest=latest,_extra=exset
...
function VERTIGO::writeheader,datamap
...
function VERTIGO::embedimg,image,filename
...
pro VERTIGO::savefits2map,filelist
...
{time:time, y1:y1, y2:y2, index:index}

index2map,indexarr,dataarr,maparr
*(self.map)=maparr

;if self->get(/instrument) eq 'VERTIGO' then maparr=self->compensate(maparr)
...
function VERTIGO::fullpath2filename,filelist
...
pro VERTIGO::loaddata,filelist,fileslist
...
function VERTIGO::checklocal,filelist
...
function VERTIGO::frameworkcheck,pointervar,filelist,wfile,wflist
...
pro VERTIGO::read,timerange=timerange,filelist=filelist,_extra=ex
...
pro VERTIGO::copy,error,filelist=filelist
...
function VERTIGO::splitpath,filelist
...
function VERTIGO::flist2tlist,filelist
...
function VERTIGO::pathogen,tname
...
function VERTIGO::timearrgen,tstart,tend;, vms=vms
...
function VERTIGO::filter,filelist,sttime,entime,_extra=exfilt
...
function VERTIGO::checklatest,setstart,tstart
...
function VERTIGO::checkfilerepeat,filelist
...
function VERTIGO::listgen,tstart,tend,range=range, settime=settime
...
function VERTIGO::list,timerange=timerange,_extra=exfilt
...
function VERTIGO::fullpath,tstart,tend
...
print,' '
print,'VERTIGO cleanup complete.'
print,' '
...
pro VERTIGO::server,status,time
